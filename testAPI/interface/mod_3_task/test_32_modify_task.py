...
@allure.feature('任务配置')
@allure.story('暂停启动任务')
@pytest.mark.parametrize('tasks', ["crowd_channel_1", "cross_channel_1"])
def test_3201_update_task_status(task_env, tasks):
    my_task = task_env(tasks)
...
def test_3202_update_task_name(task_env, tasks, task_rename):
    my_task = task_env(tasks)
    task_name = my_task.tasks_conf["taskName"]
    my_task.tasks_conf["taskName"] = task_rename
    my_task.save_task()
    time.sleep(2)
    assert my_task.get_task_attr("taskName") == task_rename
    my_task.assert_task_status('run')
    my_task.tasks_conf["taskName"] = task_name
    my_task.save_task()
...
@allure.story('取流信息匹配')
@pytest.mark.parametrize('tasks', ["crowd_channel_1", "cross_channel_1"])
def test_3203_get_video_param(task_env, tasks):
    my_task = task_env(tasks)
